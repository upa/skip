...
#ifndef LWTUNNEL_ENCAP_SKIP
	case LWTUNNEL_ENCAP_ILA:
		return "ila";
#else
	case LWTUNNEL_ENCAP_SKIP:
		return "skip";
#endif
...
#ifdef LWTUNNEL_ENCAP_SKIP
	else if (strcmp(name, "skip") == 0)
		return LWTUNNEL_ENCAP_SKIP;
#endif
...
static void print_encap_skip(FILE *fp, struct rtattr *encap)
{
	...
	if (tb[SKIP_ATTR_HOST_ADDR_FAMILY]) {
		family = rta_getattr_u32(tb[SKIP_ATTR_HOST_ADDR_FAMILY]);
		inet_ntop(family,
			  RTA_DATA(tb[family == AF_INET ?
				      SKIP_ATTR_HOST_ADDR4 :
				      SKIP_ATTR_HOST_ADDR6]),
			  buf, sizeof(buf));
		fprintf(fp, "host %s ", buf);
	}
	...
	if (tb[SKIP_ATTR_MAP_V4V6] && rta_getattr_u8(tb[SKIP_ATTR_MAP_V4V6])) {
		fprintf(fp, "map %s",
			rt_addr_n2a_rta(AF_INET6, tb[SKIP_ATTR_MAP_PREFIX]));
	}
}
			    ...
			    int *argcp, char ***argvp)
{
	...
	while (argc > 0) {
		...
		if (strcmp(*argv, "host") == 0) {
			...
			NEXT_ARG();
			if(inet_pton(AF_INET, *argv, &addr4)) {
				rta_addattr32(rta, len,
					      SKIP_ATTR_HOST_ADDR_FAMILY,
					      AF_INET);
				rta_addattr32(rta, len, SKIP_ATTR_HOST_ADDR4,
					      addr4);
			} else if (inet_pton(AF_INET6, *argv, &addr6)) {
				rta_addattr32(rta, len,
					      SKIP_ATTR_HOST_ADDR_FAMILY,
					      AF_INET6);
				rta_addattr_l(rta, len, SKIP_ATTR_HOST_ADDR6,
					      &addr6, sizeof(addr6));
			} else {
				invarg("invalid host address", *argv);
			}
			...
			NEXT_ARG();
			if (inet_pton(AF_INET6, *argv, &addr6)) {
				rta_addattr8(rta, len, SKIP_ATTR_MAP_V4V6, 1);
				rta_addattr_l(rta, len, SKIP_ATTR_MAP_PREFIX,
					      &addr6, sizeof(addr6));
			} else {
...
#else
	case LWTUNNEL_ENCAP_SKIP:
		parse_encap_skip(rta, len, &argc, &argv);
		break;
#endif
	case LWTUNNEL_ENCAP_IP6:
